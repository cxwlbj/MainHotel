...
#import "MagicNavigationController.h"

@interface MagicNavigationController ()
{
...
@implementation MagicNavigationController
...
- (void)panStateEndAnimationWithEndPoint:(CGPoint)endPoint
{
    if (endPoint.x - _startPoint.x > 50) {
        ...
        // 计算动画时间
        _animationTime = .35 - (endPoint.x - _startPoint.x) / KIPHONEWidth * .35;
        [UIView animateWithDuration:_animationTime animations:^{
            [self moveViewPopWithX:KIPHONEWidth];
        ...
        // 1.计算动画时间
        _animationTime = (endPoint.x - _startPoint.x) / KIPHONEWidth * .35;
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    [self moveViewPushWithX:KIPHONEWidth];
...
- (void)popSetAnimationView
{
    ...
    CGRect frame = CGRectMake(0, 0, KIPHONEWidth, version >= 7.0 ? KIPHONEHeight : KIPHONEHeight - 20);
...
{
    // 容错 0 < x < KIPHONEWidth
    x = MIN(x, KIPHONEWidth);
    x = MAX(x, 0);
    
    // 定义视图的缩放比例
    float scale = (x / KIPHONEWidth) * .1 + .9;
    _backImageView.transform = CGAffineTransformMakeScale(scale, scale);
    
    // 定义遮罩视图的透明度
    float alpha = .5 - (x / KIPHONEWidth) * .5;
